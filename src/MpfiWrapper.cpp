...
namespace PolynomialForms {
    ...
    MpfiWrapper operator*(const double r1, const MpfiWrapper &r2) {
        MpfiWrapper res;
        res.flag |= mpfi_mul_d(res.r, r2.r, r1);
        return res;
    }
    ...
    double MpfiWrapper::getRandomSample() const {
        double l = lower(), u = upper(), delta = u - l;
        double tmp = delta * ( (static_cast<float>(rand()))/(static_cast<float>(RAND_MAX)));
        //std::cout << "Sample Test: " << lower() << ", " << upper() << " Sample: " << l + tmp << std::endl;
        return l + tmp;
    }
